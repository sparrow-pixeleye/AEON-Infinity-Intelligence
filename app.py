...
def load_memory():
    """Safely load conversation memory."""
    try:
        with open(MEMORY_FILE, 'r', encoding='utf-8') as f:
            data = json.load(f)
            if not isinstance(data, list):
                print("⚠️ memory.json contained non-list data. Resetting it.")
                return []
            return data
...
def save_memory(messages):
    """Safely save conversation memory (keep last 20 messages)."""
    try:
        if len(messages) > 20:
...
def chat():
    try:
        ...
        if not isinstance(memory, list):
            print("⚠️ memory was not a list, resetting.")
            memory = []
        ...
        # Math calculations
            ...
            word in user_message.lower() for word in ['calculate', 'compute', 'what is', 'how much is', 'solve']
        ):
            try:
        ...
        if response is None:
            if mode == 'web':
                search = utils.perplexity_search(user_message)
                if search:
                    response = f"🔍 **Live Web Search Results:**\n\n{search}\n\n*(Information from real-time sources)*"
                    mode_used = 'web'
                else:
                    response = "❌ I couldn't retrieve current info at the moment."
                    mode_used = 'web'
        ...
        if response is None and files:
            response = "📁 I received your files. File analysis is not yet implemented."
            mode_used = 'ai'

        # AI mode via OpenRouter
        if response is None:
            openrouter_key = os.getenv('OPENROUTER_API_KEY')
            if not openrouter_key:
                response = "🔑 OpenRouter API key not configured. Please set it in environment variables."
                mode_used = 'ai'
            else:
                sys_prompt = {
                    "role": "system",
                    "content": f"""You are AEON ∞ — Infinity Intelligence by Apratim Mrinal. 
You are aware, eloquent, precise, and insightful. 
Mode: {mode}. Format responses beautifully in Markdown."""
                }
                ...
                try:
                    ...
                    response = f"❌ Connection issue: {str(e)}"
                    mode_used = 'ai'
